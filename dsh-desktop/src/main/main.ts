...
function resolveEngine(): { directory: string; cliEntrypoint: string; nodeExecutable: string } {
  ...
  if (app.isPackaged || existsSync(join(process.resourcesPath, 'engine'))) {
...
function resolveDesktopIcon(): string {
  return app.isPackaged
    ? join(process.resourcesPath, 'deepseek-icon.png')
    : join(desktopRoot, 'build', 'deepseek-icon.png')
}
...
async function configureUpdater(): Promise<void> {
  ...
  const updater = (await import('electron-updater')).autoUpdater
  autoUpdater = updater
  ...
  updater.on('update-available', update => {
    void dialog.showMessageBox(mainWindow!, {
      ...
      message: `Deepsee HARNESS ${update.version} is available.`,
      detail: 'Download the update now? It will install after you confirm a restart.',
      buttons: ['Download', 'Later'],
      defaultId: 0,
      cancelId: 1,
    }).then(result => {
      if (result.response === 0) void updater.downloadUpdate()
    })
  })
  updater.on('update-not-available', () => {
    ...
    void dialog.showMessageBox(mainWindow!, {
      ...
      message: 'Deepsee HARNESS is up to date.',
    })
  })
  updater.on('update-downloaded', update => {
    void dialog.showMessageBox(mainWindow!, {
      type: 'info',
      title: 'Update ready',
      message: `Deepsee HARNESS ${update.version} has been downloaded.`,
  ...
  updater.on('error', error => {
    ...
    void dialog.showMessageBox(mainWindow!, {
      ...
      message: 'Deepsee HARNESS could not check for updates.',
...
async function checkForUpdates(requestedByUser: boolean): Promise<void> {
  if (!updatesEnabled || autoUpdater === undefined) {
    if (requestedByUser) {
...
function createWindow(): void {
  ...
  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL) => {
    console.error(`Desktop window failed to load (${String(errorCode)}): ${errorDescription} (${validatedURL})`)
  })
...
app.whenReady().then(() => {
  ...
  void configureUpdater().then(() => checkForUpdates(false)).catch(error => {
    updatesEnabled = false
    console.error(`The Deepsee HARNESS updater could not start: ${error instanceof Error ? error.message : String(error)}`)
  })
  app.on('activate', () => {
